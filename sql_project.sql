...
select customer.customer_id,first_name,last_name, sum(total) as max_buys
from customer
join invoice
on customer.customer_id=invoice.customer_id
group  by customer.customer_id
order by max_buys desc
limit 1;
...
WITH best_selling_artist as(
    select artist.artist_id as artist_id,artist.name as artist_name,sum(invoice_line.unit_price*invoice_line.quantity) as total_sails
	from invoice_line
...
SELECT c.customer_id,c.first_name,c.last_name,bsa.artist_name,sum(il.unit_price*il.quantity)as total_sails
from invoice i
...
with best_selling_genre as (
	select count(invoice_line.quantity) as purches, genre.name,genre.genre_id,customer.country,
